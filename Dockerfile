...
# Download and compile Python 3.13 from source
RUN wget https://www.python.org/ftp/python/3.13.1/Python-3.13.1.tgz && \
    tar xzf Python-3.13.1.tgz && \
    cd Python-3.13.1 && \
    ./configure --enable-optimizations --enable-shared --with-ensurepip=install --prefix=/usr/local LDFLAGS="-Wl,-rpath,/usr/local/lib" && \
    make -j$(nproc) && \
    make altinstall && \
    cd .. && \
